...
export class ProjetEditAdminComponent extends AbstractEditController<ProjetDto, ProjetCriteria, ProjetService>   implements OnInit {
    ...
    public prepareEdit() {
        this.item.dateDebut = this.projetService.format(this.item.dateDebut);
        this.item.dateFinEstime = this.projetService.format(this.item.dateFinEstime);
        this.item.dateFin = this.projetService.format(this.item.dateFin);
    }



    public validateProjetRessources(){
        this.errorMessages = new Array();
        this.validateProjetRessourcesCode();
    }
    public validateTaches(){
        this.errorMessages = new Array();
        this.validateTachesCode();
    }
    public validateRemarques(){
        this.errorMessages = new Array();
   ...
   public addProjetRessources() {
       ...
       if (this.errorMessages.length === 0) {
            if(this.projetRessourcesElement.id == null){
                this.item.projetRessources.push(this.projetRessourcesElement);
            }else{
                const index = this.item.projetRessources.findIndex(e => e.id == this.projetRessourcesElement.id);
                this.item.projetRessources[index] = this.projetRessourcesElement;
            }
   ...
   public addTaches() {
       ...
       this.validateTaches();
       if (this.errorMessages.length === 0) {
            if(this.tachesElement.id == null){
                this.item.taches.push(this.tachesElement);
            }else{
                const index = this.item.taches.findIndex(e => e.id == this.tachesElement.id);
                this.item.taches[index] = this.tachesElement;
            }
   ...
   public addRemarques() {
       ...
       if (this.errorMessages.length === 0) {
            if(this.remarquesElement.id == null){
                this.item.remarques.push(this.remarquesElement);
            }else{
                const index = this.item.remarques.findIndex(e => e.id == this.remarquesElement.id);
                this.item.remarques[index] = this.remarquesElement;
            }
    ...
    private validateProjetRessourcesCode(){
   ...
   public async openCreateTypeProjet(typeProjet: string) {
        const isPermistted = await this.roleService.isPermitted('TypeProjet', 'edit');
        if(isPermistted) {
             this.typeProjet = new TypeProjetDto();
   ...
   public async openCreateChefProjet(chefProjet: string) {
        const isPermistted = await this.roleService.isPermitted('ChefProjet', 'edit');
        if(isPermistted) {
             this.chefProjet = new ChefProjetDto();
