...
@Component({
  selector: 'app-materiel-edit-admin',
  templateUrl: './materiel-edit-admin.component.html'
})
export class MaterielEditAdminComponent extends AbstractEditController<MaterielDto, MaterielCriteria, MaterielService>   implements OnInit {
   ...
   public async openCreateCategorieMateriel(categorieMateriel: string) {
        const isPermistted = await this.roleService.isPermitted('CategorieMateriel', 'edit');
        if(isPermistted) {
             this.categorieMateriel = new CategorieMaterielDto();
