...
@Component({
  selector: 'app-declaration-edit-admin',
  templateUrl: './declaration-edit-admin.component.html'
})
export class DeclarationEditAdminComponent extends AbstractEditController<DeclarationDto, DeclarationCriteria, DeclarationService>   implements OnInit {
   ...
   public async openCreateTypeDeclaration(typeDeclaration: string) {
        const isPermistted = await this.roleService.isPermitted('TypeDeclaration', 'edit');
        if(isPermistted) {
             this.typeDeclaration = new TypeDeclarationDto();
